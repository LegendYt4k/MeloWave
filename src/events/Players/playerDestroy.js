...
module.exports = {
    ...
    run: async (client, player) => {
        ...
        const web2 = client.channels.cache.get(client.config.dlogs);
 

const server = client.guilds.cache.get(player.guild);

const embed = new MessageEmbed().setColor(client.embedColor).setAuthor("Player Destroyed", client.user.displayAvatarURL()).setDescription(`ServerName: **${name}**`)

web2.send({embeds: [embed]});
